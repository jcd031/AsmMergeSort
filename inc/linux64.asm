...
%macro print 1                ; print(string)
      mov rax, %1
%%printString:                 ; print(rax), rax as pointer to string
      push rax
      mov rbx, 0              ; Will be used to count the length of the string
%%printLoop:
      inc rax                 ; Point to next char
      inc rbx                 ; Inc count
      mov cl, [rax]
      cmp cl, 0               ; Check if the new byte is the sentinel value
      jne %%printLoop          ; If it isn't loop

      mov rax, SYS_WRITE       ; Print to system output
      mov rdi, STDOUT
      pop rsi                 ; Pop rax argument into the buffer register
      mov rdx, rbx            ; Length = count
      syscall
%endmacro
...
%macro printNumber 1          ; printNumber(int)
      mov rax, %1
%%printNumber:
      mov rcx, digitSpace     ; rcx = &digitSpace[0]
      mov rbx, 10             ; rbx = Sentinel value
      mov [rcx], rbx          ; digitSpace[0] = rbx
      inc rcx                 ; rcx = &digitalSpace[1]
      mov [digitSpacePos], rcx; digitSpacePos = &digitSpace[1]

%%printNumberLoop:
      mov rdx, 0              ; Ensure rdx gets used for remainder
      mov rbx, 10             ; We're dividing by 10
      div rbx
      push rax
      add rdx, 48             ; Shifting up for ASCII

      mov rcx, [digitSpacePos]; rcx = digitSpace[current]
      mov [rcx], dl           ; digitSpace[current] = remainder
      inc rcx                 ; rcx = &digitSpace[next]
...
%%printNumberLoop2:
      ...
      mov rax, SYS_WRITE      ; Print the last(first) character on the string
      mov rdi, STDOUT
      mov rsi, rcx
      mov rdx, 1
      syscall
      ...
      mov rcx, [digitSpacePos]
      dec rcx
      mov [digitSpacePos], rcx; Shift down to the next char

      cmp rcx, digitSpace     ; if the current index >= index 0:
      jge %%printNumberLoop2   ; continue the loop
%endmacro

;--------------------------------------------------------------------------------
;     countChars(string * rax)
;     {
;           while(*rax != 0)
;           {
;                 rbx++;
...
;     }
;--------------------------------------------------------------------------------
%macro countChars 1           ; Get string and store the length in rdx
      mov rax, %1
_countChars:                  ; Takes rax as pointer to string
      xor rdx, rdx            ; Set rbx to 0
      mov cl, [rax]
      cmp cl, 0
      je _countCharsEnd
_countCharsLoop:
